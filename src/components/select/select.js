...
export default function CountrySelect({ selectedCountry, onChange }) {
    ...
    useEffect(() => {
        const fetchCountries = async () => {
            try {
                ...
                // Convert the API response into an array of objects with `name` and `code`
    ...
    return (
        <Listbox value={selectedCountry} onChange={handleChange}>
            <Label className="block text-sm font-medium text-gray-900">Davlatni tanlang</Label>
                    ...
                    {countries.length > 0 ? (
                        <>
                            ...
                            <ListboxOption
                                value={null}
                                disabled
                                className="text-gray-400 cursor-default select-none py-2 pl-3 pr-9"
                            >
                                Select a country
                            </ListboxOption>
